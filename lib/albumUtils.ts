...
/**
 * Creates a single "photo album" page image from a collection of region images.
 * @param imageData A record mapping region strings to their image data URLs.
...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    const regions = Object.keys(imageData);
    const loadedImages = await Promise.all(
        Object.values(imageData).map(url => loadImage(url))
    );

    const imagesWithRegions = regions.map((region, index) => ({
        region,
    ...
    imagesWithRegions.forEach(({ region, img }, index) => {
        ...
        ctx.fillText(region, 0, captionY);
