...
const POSITIONS = [
    { top: '2%', left: '5%', rotate: -10 },
    { top: '5%', left: '35%', rotate: 8 },
    { top: '3%', left: '65%', rotate: -5 },
    { top: '30%', left: '15%', rotate: 12 },
    { top: '35%', left: '50%', rotate: -3 },
    { top: '28%', left: '75%', rotate: 15 },
    { top: '55%', left: '5%', rotate: 5 },
    { top: '60%', left: '38%', rotate: -12 },
    { top: '58%', left: '68%', rotate: 10 },
...
function App() {
    ...
    const handleGenerateClick = async () => {
        ...
        const processEra = async (era: string) => {
            try {
                const prompt = `Reimagine the person in this photo as if they were from the ${era} era. This should be a celebration of that era's culture, including representative clothing, hairstyle, and the overall aesthetic. The output must be a photorealistic, historically-inspired image showing the person clearly, with a respectful and authentic representation.`;
    ...
    const handleRegenerateEra = async (era: string) => {
        ...
        if (generatedImages[era]?.status === 'pending') {
            return;
        }
        
        console.log(`Regenerating image for ${era}...`);

        // Set the specific era to 'pending' to show the loading spinner
        setGeneratedImages(prev => ({
            ...prev,
            [era]: { status: 'pending' },
        }));

        // Call the generation service for the specific era
        try {
            const prompt = `Reimagine the person in this photo as if they were from the ${era} era. This should be a celebration of that era's culture, including representative clothing, hairstyle, and the overall aesthetic. The output must be a photorealistic, historically-inspired image showing the person clearly, with a respectful and authentic representation.`;
            const resultUrl = await generateStyledImage(uploadedImage, prompt);
            setGeneratedImages(prev => ({
    ...
    const handleDownloadIndividualImage = (era: string) => {
        ...
        if (image?.status === 'done' && image.url) {
            ...
            link.href = image.url;
            link.download = `past-forward-${era.toLowerCase().replace(/\s+/g, '-')}.jpg`;
    ...
    // FIX: Add 'as const' to the object to fix a TypeScript error with framer-motion's Transition type.
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
                <div className="text-center mb-10">
                    ...
                    <p className="font-permanent-marker text-neutral-300 mt-2 text-xl tracking-wide">See yourself reimagined through the ages.</p>
                </div>
            ...
            <AnimatePresence>
                {fullscreenImage && (
                    <motion.div
                        ...
                        className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 sm:p-8 cursor-zoom-out"
                    >
                        <motion.div
                             key="fullscreen-image-container"
                        ...
                        >
                            <img
                                src={fullscreenImage}
                                alt="Fullscreen view of generated image"
                        ...
                        <motion.button
                            ...
                            exit={{ opacity: 0 }}
                            onClick={handleCloseFullscreen}
                            className="absolute top-4 right-4 text-white bg-black/50 p-2 rounded-full hover:bg-black/75 transition-colors focus:outline-none focus:ring-2 focus:ring-white"
